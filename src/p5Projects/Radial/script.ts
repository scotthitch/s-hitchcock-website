...
const script = (p5Instance: P5): void => {
    ...
    // Setup the canvas
    p5Instance.setup = () => {
        p5Instance.createCanvas(innerWidth, window.innerHeight)
    ...
    p5Instance.draw = () => {
        ...
        for (let i = 0; i < numberOfReflections; i++) {
            ...
            p5Instance.stroke(hue, DRAWING_SATURATION, DRAWING_LIGHT)
            p5Instance.line(
                prevTranslatedPolarMouse.r * p5Instance.cos(prevTranslatedPolarMouse.theta + theta), // x1
                prevTranslatedPolarMouse.r * p5Instance.sin(prevTranslatedPolarMouse.theta + theta), // y1
