...
const script = (screenDimensions: ScreenDimensions): p5ScriptInnerFunction => {
    const s = (p5Instance: P5): void => {
        ...
        function generatePerlinForcePatch(i: number, j: number) {
            ...
            const angle = p5Instance.noise(i * NOISE_SCALE, j * NOISE_SCALE) * CURL + RANDOM_BASE_OFFSET; // Adjust noise parameters
        ...
        p5Instance.setup = () => {
            ...
            for (let i = 0; i < N_POINTS; i++) {
                ...
                const pointPos = p5Instance.createVector(Math.random() * p5Instance.width, Math.random() * p5Instance.height);
        ...
        class Point {
            ...
            resetPoint() {
                ...
                this.pos = p5Instance.createVector(Math.random() * this.screenBounds.x, Math.random() * this.screenBounds.y);
                this.vel = p5Instance.createVector(0, 0);
                this.acc = p5Instance.createVector(0, 0);
            }
